...
# binomial test
binom.test(prfs)
# Exact test, but costs more computationally
#Can only handle two responsive categories. 
...
#Sequential P Value that it adjusts for depending on sample size. 
# If we are using multiple tests than we need to correct for the p value by using this method. 
...
# T statistic, is the value of the t distribution that we are getting from the data
#Degrees of freedom -1 minus from the surveyed total amount
#We have a significant difference between the number of pages visited from the websites a and b

#Parametric vs non parametric tests.

# pgviews: t(498)=-7.21, p <0.0001
...
#Analysis of variants there are 3 ANOVS:
#Independence
#Normality 
#Homoscedasticity - Homogeneity of Variants. Spread very similiar?
